...
- `V1`~`V6`에는 User/Space/Meeting/TranscriptSegment/Report/Knowledge/EmbeddingChunk/Task schema가 있다. 공유된 migration은 수정하지 않고 보강은 `V7` 이후로만 추가한다.
...
3. `V7` 이후 migration으로 인증 세션/초대, 전사 상태·보존, 용어사전·감사 로그, embedding job/generation 누락을 순차 보강한다.
