...
- Workspace와 Backend가 저장하는 AI artifact의 domain model 자체에 JPA mapping을 둔다. 별도 `persistence/entity` record 변환 계층을 만들지 않고, `JpaWorkspacePersistence`가 동일 도메인 엔티티를 직접 영속화한다. embedding job/chunk worker는 native SQL/JDBC 경계를 유지한다.
- JPA dependency, Flyway schema validation, Space/Meeting/ACL/Transcript/Report/Task/Knowledge entity mapping과 `JpaWorkspaceStore`를 적용한다. JDBC `WorkspaceStore`는 Auth 및 PostgreSQL round-trip 검증 helper로만 유지한다.
