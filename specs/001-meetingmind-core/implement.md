...
- 2026-07-09: `MeetingParticipant.accessStatus` canonical 값은 `ACTIVE`, `REVOKED`로 status-values, data-model, ERD, contracts에 반영했다. SpaceMember 제거 시 MeetingParticipant 처리 정책은 2026-07-10 M020 보정에서 프로젝트 권한 제거와 회의 단독 권한 유지를 분리하는 방향으로 갱신했다.
...
- 2026-07-09: T036/T037로 `backend/src/main/java/com/meetingmind/demo/domain/**` 최소 도메인 record와 `InMemoryWorkspaceStore`, `WorkspaceDomainService`를 추가했다. Space 생성은 생성자를 `OWNER` SpaceMember로 등록하고, 회의 생성은 `OWNER`/`ADMIN`만 허용하며 생성자를 `HOST` MeetingParticipant로 등록한다. 추가 참여자 처리는 M020 보정 이후 SpaceMember 여부와 프로젝트 접근권 생성을 분리한다.
...
- Passed: `cd backend && ./gradlew test` after M021 meeting join request approval flow, total 64 backend tests
- Passed: `git diff --check` after M021 meeting join request approval flow
