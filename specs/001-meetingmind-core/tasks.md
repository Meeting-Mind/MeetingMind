...
- [x] T455 [fix/stt-startup] [owner: Claude] 독립 STT 서비스가 기동하지 못하던 문제를 고친다. `TranscriptionCoordinator`에 생성자가 둘인데 `@Autowired`가 없어 Spring이 주입 대상을 고르지 못했다. STT 테스트가 전부 단위 테스트여서 조립 단계를 아무도 보지 않고 있었다(`T450.1`과 같은 계열). 재발 방지는 컨텍스트 기동 테스트 대신 **reflection으로 모든 bean의 생성자 주입 가능성을 검사**한다 — DB 없이 끝나 CI 시간을 늘리지 않으면서 bean 전체를 덮는다. 수정을 되돌린 상태에서 먼저 실패시켜 검사가 유효함을 확인했다.
- [x] T439.4 [observability/backend] [owner: Claude] [depends: T439.2] STT/LiveKit/report-confirm custom metric을 추가하고 Grafana STT/Live 대시보드를 연결한다. 계측은 `ConfiguredTranscriptionGateway`에 두어 gateway mode를 바꿔도 지표가 끊기지 않게 한다. label은 `outcome`만 둔다(식별자는 cardinality 폭발 + `NFR-LOG-01` 위반). 성공/실패를 모두 기록하고 예외는 다시 던진다. dashboard 쿼리의 지표 이름과 테스트가 고정한 이름을 기계적으로 대조해 누락을 잡는다.
- [x] T439.2 [observability/grafana] [owner: Claude] [depends: T439] Grafana datasource/dashboard provisioning과 dashboard JSON을 `infra/grafana/`에 추가한다. AI 지표 8종은 실제 `/metrics` 출력과 대조해 존재를 확인했고, BFF 지표는 Micrometer 등록 코드 기준 변환 규칙을 적용했다. STT/LiveKit 패널은 metric이 없어 제외한다 — 없는 지표는 오류가 아니라 빈 패널이 되어 "대시보드가 있다"는 착각만 남긴다. 부수로 BFF `/actuator/prometheus`가 테스트 환경에서 Prometheus 형식이 아님을 발견했다(`T439.3`).
- [x] T439.3 [observability/bff] [owner: Claude] [depends: T439.2] BFF `/actuator/prometheus` 노출 형식 검증. 원인은 **운영 결함이 아니라 테스트 설정**이었다. Spring Boot는 테스트에서 metrics export auto-configuration을 기본으로 끄고, `PrometheusScrapeController`에는 registry가 없을 때 동작하는 fallback이 있어 Prometheus가 파싱할 수 없는 텍스트를 반환하고 있었다. `@AutoConfigureObservability`를 붙여 실제 형식을 확인하고, dashboard가 쓰는 지표 이름 5종을 고정하는 테스트를 추가했다.
- [x] T439.1 [observability/frontend-backend] [owner: Claude] [depends: T439] Space 단위 AI Usage/quota. API와 `Knowledge Indexed` 제거는 이미 되어 있었고 `limit`/`usagePercent`가 null 하드코딩이며 화면 연결이 없었다. quota 상한은 `MEETINGMIND_AI_TOKEN_QUOTA` 전역 기본값에서 오고, 미설정이면 `limit`/`usagePercent`를 둘 다 null로 둔다(0은 "한도 0"으로 오해된다). **표시 전용이며 초과해도 AI 호출을 차단하지 않는다** — 프로토타입에서 설정 실수로 팀 전체가 막히는 위험을 지지 않는다. Space 개요에 사용량 카드를 붙이고, 조건부 렌더가 조용히 사라지는 것을 잡기 위해 e2e를 함께 둔다.
- [x] V119 [verification] [owner: QA/Integration] [depends: T435,T436,T438,T439] 요구사항 ID별 상태를 갱신하고 실행 결과를 `implement.md`에 기록했다. `SMK-001`~`SMK-005`의 **자동 검증 가능한 범위는 전부 닫혔다**(`T445`~`T449`, `T453`, `T454`). 착수 시점에 수동으로 남아 있던 2건 중 `SMK-005` UI 우회 경로는 `T453`, `SMK-002` 매체 publish/subscribe는 `T454`로 자동화했다. **미수행 이월 1건**: fake media가 대신할 수 없는 장치 권한 프롬프트·prejoin 장치 선택 UX·실제 음성 품질 확인은 수행되지 않았으며, 소유자 판단으로 V119 마감 조건에서 제외하고 별도 항목으로 이월한다(통과로 기록하지 않는다).
...
- [x] T454 [smoke/live-media] [owner: Claude] [depends: T444] `SMK-002` 매체 축을 Playwright 2 context로 자동화한다. `T444`는 서버 도달성만 덮고 브라우저 client가 없어 publish/subscribe가 미검증이었다. Chromium fake device로 두 참가자가 같은 회의에 들어가 서로를 원격 참가자로 보는지 확인하고, `No other participants` -> 상대 등장 전이를 함께 단정한다. UI 전에 livekit-token 발급 성공을 선단정한다. CI에 `backend/.env`가 없어 `RUN_LIVEKIT_MEDIA_E2E=true` 명시적 opt-in으로 둔다(조건부 skip은 통과처럼 보이므로 쓰지 않는다). 장치 권한 프롬프트·prejoin 장치 선택·실제 음성 품질은 fake media로 덮이지 않아 수동으로 남는다.
- [x] T453 [smoke/guest-acl-ui] [owner: Claude] [depends: T446] `SMK-005` 브라우저 축을 Playwright로 자동화한다. 수동 필수로 두고 있었으나 `playwright.config.ts`가 backend/BFF/frontend를 함께 띄우고 CI job이 `e2e/` 전체를 실행하고 있었다. 회의 전용 GUEST가 URL 직접 입력으로도 Space 범위 화면에 도달하지 못하고 API도 거부됨을 확인한다. 음성 단정이 `toHaveCount(0)`이라 공허하게 통과할 수 있어 소유자 양성 대조를 함께 둔다. backend가 `test` 프로파일(in-memory)이므로 SQL 축은 `T446`이 담당한다 — 두 축은 서로를 대신하지 못한다.
- [x] T452 [obs/embedding-jobs] [owner: Claude] [depends: T447] `embedding_jobs`에 실패 원인을 남긴다. `INTERNAL_ERROR`가 미분류 예외의 포괄 코드인데 원인을 담을 컬럼이 없어 실패 행만으로는 진단이 불가능했다. `V25`로 `failure_detail`을 추가하고 worker가 예외의 정규화된 **타입 이름만** 기록한다. 예외 메시지는 provider 응답이나 DSN을 실어올 수 있어 저장하지 않는다(`NFR-LOG-01`). `log_event`의 `_SAFE_FIELDS` allowlist에도 추가해야 로그에서 조용히 버려지지 않는다.
- [x] T451 [fix/ai-context] [owner: Claude] `AH-009` shrink 순서 결함을 고친다. `format_untrusted_sources`가 위치로만 잘라, Backend가 발화 순서로 보내는 리포트/task 경로에서 높은 score 근거가 먼저 버려졌다. 기존 테스트는 15건의 score를 전부 동일하게 두어 순서 정책을 검증하지 않았다. 절단 지점에서 score 정렬 후 자르도록 바꾸고 회귀 테스트를 추가한다. token 단위 회계는 여전히 없다(`T451.1`).
- [x] T451.1 [feature/ai-context] [owner: Claude] [depends: T451] `AH-009` token 기반 budget을 구현한다. `estimate_tokens`는 문자 기반 보수적 추정이며(`tiktoken` 의존성을 늘리지 않는다) CJK를 과대평가해 안전한 방향으로 틀린다. `AI_CONTEXT_TOKEN_BUDGET` 전역 기본값과 `AI_CONTEXT_TOKEN_BUDGET_<FEATURE>` 기능별 덮어쓰기로 `PERF-TOKEN-01`의 예산 분리 요건을 만족한다. 예산이 아무리 작아도 최소 1건은 남긴다 — 전부 버리면 `NO_EVIDENCE`(검색 실패 신호)와 구분되지 않기 때문이다.
