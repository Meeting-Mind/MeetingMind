...
| Medium | Backend 권한 policy가 M020에서 주요 runtime mutation/API에 연결됐다. | SpaceMember 제거, participant revoke, owner transfer, Project AI context 후보 조회가 frontend local state 표현을 넘어서 backend 보안 경계에서도 검증된다. DB 영속화 전까지 audit는 in-memory event다. | 후속 DB/API milestone에서 Space invitation/MeetingJoinRequest persistence, Kanban persistence, report/task candidate API, AuditLog 영속화를 이어간다. | `backend/src/main/java/com/meetingmind/demo/authz/**`, `backend/src/main/java/com/meetingmind/demo/domain/**`, `backend/src/main/java/com/meetingmind/demo/controller/**`, `contracts/space-api.md`, `contracts/meeting-api.md`, `contracts/ai-api.md`, `tasks.md`, `plan.md`, `implement.md` | Backend Runtime Verified |
| Medium | M021에서 URL/코드 기반 회의 참가 신청과 HOST 승인 runtime을 구현했다. | 승인 전 접근 차단, 회의 단독 guest 권한, Space membership 비생성, 검토 권한과 상태 재처리 차단이 Backend에서 강제된다. 현재 코드는 in-memory raw joinCode 저장이다. | DB 전환 시 joinCode hash와 MeetingJoinRequest partial unique index를 추가하고, M022 Frontend와 실제 계정/meeting E2E를 검증한다. | `requirements/*`, `backend/src/main/java/com/meetingmind/demo/**`, `contracts/meeting-api.md`, `data-model.md`, `erd.md`, `tasks.md`, `implement.md` | Backend Runtime Verified |
| Medium | M022에서 Frontend URL/code 신청, 권한 확인, HOST 승인, prejoin/LiveKit default-deny gate를 구현했다. | 사용자는 SpaceRole과 meeting ACL을 분리해 확인하고, 승인 전에는 회의 media/room 진입을 시도할 수 없다. local 승인과 SpaceMember 제거 semantics도 Backend 정책과 일치한다. | Backend와 유효한 실제 계정/meeting을 함께 실행해 applicant→HOST approval→prejoin→LiveKit E2E와 desktop/mobile visual smoke를 추가 검증한다. | `frontend/src/pages/MeetingAccessPage.tsx`, `LiveMeetingPage.tsx`, `LiveRoomPage.tsx`, `TeamMembersPage.tsx`, `App.tsx`, `tasks.md`, `implement.md` | Frontend Build Verified |

## Recommendation

1. M019 잔여 frontend report/task candidate 작업을 이어간다.
2. Meeting AI 컨텍스트 조립을 Backend 권한 검증 뒤로 이동한다.
3. Space invitation/MeetingJoinRequest persistence, Kanban persistence, report/task candidate API, AuditLog 영속화를 후속 backend/data milestone로 분리한다.
