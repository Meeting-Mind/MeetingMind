...
| Medium | M018 Project Workspace Frontend는 local state 기준으로 완료됐고 M027에서 권한 보안 경계를 Backend mutation/API로 연결했다. | SpaceMember 제거, participant revoke, owner transfer, Project AI context 후보 조회가 frontend 표현을 넘어 backend에서도 검증된다. DB 영속화 전까지 audit는 in-memory event다. | 후속 DB/API milestone에서 Space invitation/MeetingJoinRequest persistence와 AuditLog 영속화를 이어간다. | `backend/src/main/java/com/meetingmind/demo/authz/**`, `backend/src/main/java/com/meetingmind/demo/domain/**`, `backend/src/main/java/com/meetingmind/demo/controller/**`, `contracts/space-api.md`, `contracts/meeting-api.md`, `tasks.md`, `plan.md`, `implement.md` | M027 Verified |
| Medium | M028에서 URL/코드 기반 회의 참가 신청과 HOST 승인 runtime을 구현했다. | 승인 전 접근 차단, 회의 단독 guest 권한, Space membership 비생성, 검토 권한과 상태 재처리 차단이 Backend에서 강제된다. 현재 코드는 in-memory raw joinCode 저장이다. | DB 전환 시 joinCode hash와 MeetingJoinRequest partial unique index를 추가하고 실제 계정/meeting E2E를 검증한다. | `requirements/*`, `backend/src/main/java/com/meetingmind/demo/**`, `contracts/meeting-api.md`, `data-model.md`, `erd.md`, `tasks.md`, `implement.md` | M028 Verified |
...
4. Data/Backend 영속화 이후 MeetingJoinRequest, AuditLog, PostgreSQL/pgvector retriever와 embedding worker를 구현한다.
