...
| High | Core의 Browser token 계약은 현재 구현됐지만 기업 운영 목표에서는 superseded됐다. | legacy `sessionStorage`를 새 기능이 재사용하면 P0 token 비저장 정책과 충돌한다. | 현재 계약은 BFF adapter/rollback으로만 보존하고 신규 인증은 `../002-bff-auth-msa/**`를 따른다. | `clarify.md`, `contracts/auth-api.md`, `data-model.md`, `../002-bff-auth-msa/**` | Superseded for target |
