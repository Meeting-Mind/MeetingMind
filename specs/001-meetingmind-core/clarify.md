...
- D-022: 회의 생성 시 추측하기 어려운 `joinCode`를 발급한다. 인증 사용자는 회의 URL 또는 코드만으로 `PENDING` 참가 신청을 만들 수 있고, active `HOST`가 승인하면 기본 `VIEWER` MeetingParticipant가 생성된다. Space OWNER/ADMIN은 ACL 관리 override로 승인/거절할 수 있으며, 승인 전에는 회의 접근권이나 SpaceMember가 생기지 않는다.
