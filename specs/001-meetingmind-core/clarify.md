...
- D-040: Auth는 기존 JDBC `AuthStore`와 `users` 테이블 관리 경계를 유지한다. Auth 이외 Workspace와 AI artifact 도메인은 JPA entity/repository adapter로 단계 전환한다. 전환 중에도 Flyway만 schema owner이며 `ddl-auto=validate`를 사용한다. `user_id` 계열 FK는 Auth entity 연관관계가 아닌 scalar ID로 유지하고, pgvector 검색/worker의 `embedding_chunks`는 native SQL/JDBC 경계를 유지한다.
