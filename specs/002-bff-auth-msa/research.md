...
- 목표는 브라우저 토큰 노출을 제거하면서 AWS ECS Fargate에서 Web BFF, Auth, Core, AI, Realtime 장애를 점진적으로 격리하는 것이다.

## Questions

- RQ-001: 브라우저 token 탈취를 줄이면서 서비스 분리 후 사용자 권한을 어떻게 전달할까?
- RQ-002: 현재 모놀리스에서 MSA로 big-bang 없이 이동할 수 있는 경계는 무엇인가?
- RQ-003: 세션과 token을 어디에 저장해야 ECS Fargate 수평 확장과 즉시 로그아웃을 함께 지원할까?
...
| D-003 | BFF Redis session + 암호화 Token Vault | 다중 Task 세션 공유와 token 역할 분리를 지원한다. | JDBC session, client-side encrypted session | Redis/KMS 인프라와 장애 대응이 필요하다. |
...
| D-007 | AWS ECS Fargate 단일 리전 Multi-AZ | Kubernetes control plane과 EC2 node 운영 없이 현재 팀 규모에서 서비스별 독립 배포·스케일링·장애 격리를 제공한다. 기존 EKS 결정은 ADR 002가 대체한다. | EKS, ECS on EC2, multi-region | Kubernetes 이식성과 세밀한 오케스트레이션 기능은 줄지만 ECS Service/Task Definition, ALB, IAM, Security Group으로 현재 요구를 충족한다. |
| D-008 | LiveKit Cloud | 미디어 plane 운영과 UDP/TURN 장애를 MeetingMind ECS Fargate 경계에서 분리한다. | ECS/EC2 자체 호스팅 | provider 장애 시 실시간 기능 degradation이 필요하다. |
| D-009 | 서비스별 데이터 소유 | 공유 DB가 장애/배포 결합점이 되는 것을 줄인다. | 모든 서비스 shared schema | Auth DB 분리와 cross-service transaction 대체가 필요하다. |
| D-010 | AES-256-GCM envelope encryption + AWS KMS data key | payload 기밀성과 무결성을 함께 보장하고, ECS 서비스별 Task Role로 KMS 권한을 분리하며 저장소에는 ciphertext와 encrypted data key만 남긴다. | token 직접 KMS Encrypt, 애플리케이션 고정키 직접 암호화 | AWS SDK v2 KMS adapter와 local/test key adapter, encryption context 및 fail-closed 테스트가 필요하다. |
...
| D-018 | mTLS SPIFFE workload identity | shared secret 없이 BFF/Auth/Resource workload를 상호 인증하고 ECS 서비스 분리 후 principal 단위 최소 권한을 적용한다. | OAuth client credentials, 자체 요청 서명 | Security Group, public route 차단, 인증서 자동 회전과 principal allowlist가 필요하다. 구체 제품과 service discovery 방식은 Q-023에서 정한다. |
...
- 기존 EKS 검토에서는 security, networking, autoscaling, upgrade와 application reliability가 별도 Kubernetes Day-2 책임이라는 점을 확인했고, 현재 팀 규모에서는 이 운영 복잡도를 피하는 것이 ECS Fargate 전환 근거가 됐다: https://docs.aws.amazon.com/eks/latest/best-practices/introduction.html
...
- AWS EKS: Kubernetes 이식성과 세밀한 오케스트레이션에는 유리하지만 control plane/Kubernetes 운영과 node 전략까지 관리해야 한다. 현재 BFF/Auth/Core/AI 규모는 ECS Fargate의 서비스별 배포·autoscaling·IAM/SG 격리로 충족되므로 ADR 002에서 대체했다.
...
- 서비스별 SLO/RTO/RPO와 장애 주입 테스트를 ECS Service 출시 전에 확정한다.
- ECS 내부 service discovery와 mTLS/SPIFFE 구현 제품을 Q-023/T040에서 확정한다.
- 기존 VPC/subnet/route table Terraform은 재사용하고 수작업 NonProd 검증 뒤 ECS/ALB/IAM/Security Group/CloudWatch를 환경별 Terraform 모듈로 전환한다.
