...
- D-023: NonProd는 단일 ECS Fargate 클러스터에서 BFF/Auth/Core/AI를 별도 ECS Service/Task Definition/Task Role/Security Group/7일 Log Group으로 격리한다. `realtime-stt`는 독립 ECR 경계를 유지하되 현재 배포를 보류한다. Frontend는 S3+CloudFront+WAF, API는 Route 53→CloudFront/WAF→public ALB→BFF를 사용한다.
