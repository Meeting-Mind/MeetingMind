...
type ProjectAiSourceCandidate = {
  sourceId: string;
  sourceType: "projectKnowledge" | "meetingRecord";
  title: string;
  text: string;
};
...
export function ProjectOverviewPage({
  ...
  const defaultDeniedMembers = selectedMeeting
    ...
    : [];
  const projectKnowledgeSources: ProjectAiSourceCandidate[] = [
    {
      sourceId: `${viewData.selectedSpace.id}:knowledge:decision`,
      sourceType: "projectKnowledge",
      title: "공식 Project Knowledge · 최종 결정",
      text: viewData.knowledge.finalDecision
    },
    {
      sourceId: `${viewData.selectedSpace.id}:knowledge:owner`,
      sourceType: "projectKnowledge",
      title: "공식 Project Knowledge · 담당 구조",
      text: viewData.knowledge.ownerStructure
    }
  ];
  const accessibleMeetingSources: ProjectAiSourceCandidate[] = viewData.meetings
    .filter((meeting) => {
      const meetingKey = buildMeetingKey(selectedProjectName, meeting.index);
      const storedParticipants = meetingParticipants[meetingKey];
      const candidateParticipants = storedParticipants ?? buildDefaultMeetingParticipants(members, meetingKey);

      return candidateParticipants.some((participant) => participant.accessStatus === "ACTIVE");
    })
    .map((meeting) => ({
      sourceId: `${viewData.selectedSpace.id}:meeting:${meeting.id ?? meeting.index}`,
      sourceType: "meetingRecord" as const,
      title: `회의기록 ${meeting.index} · ${meeting.title}`,
      text: getMeetingDescription(meeting)
    }));
  const projectAiSourceCandidates = [...projectKnowledgeSources, ...accessibleMeetingSources];
  const accessibleMeetingSourceIds = new Set(
    accessibleMeetingSources.map((source) => source.sourceId.replace(`${viewData.selectedSpace.id}:meeting:`, ""))
  );
  const accessibleMeetings = viewData.meetings.filter((meeting) =>
    accessibleMeetingSourceIds.has(meeting.id ?? meeting.index)
  );
  ...
  const projectAiPayload = {
    projectId: viewData.selectedSpace.id,
    question: input.trim(),
    projectKnowledge: projectKnowledgeSources.map((source) => ({
      sourceId: source.sourceId,
      title: source.title,
      text: source.text
    })),
    meetings: accessibleMeetingSources.map((source) => ({
      meetingId: source.sourceId,
      title: source.title,
      summary: source.text
    }))
  };
  const legacyProjectAiPayload = {
    transcript: accessibleMeetingSources.map((source, index) => ({
      time: `06:${String(2 + index * 4).padStart(2, "0")}:00`,
      speaker: source.title,
      text: source.text
    })),
    decisions: projectKnowledgeSources.map((source) => ({
      title: source.text,
      meta: source.title
    })),
    actions: [
      {
        title: `${viewData.selectedSpace.name} 접근 가능한 회의 기록 ${accessibleMeetingSources.length}건 검토`,
        meta: "Project AI context"
      }
    ]
  };
  ...
  const canConfirmMeetingDelete =
    Boolean(selectedMeeting) && meetingDeleteCandidate === selectedMeeting?.index && meetingDeleteConfirm === meetingDeleteToken;
  ...
  async function askProjectAi(question: string) {
    ...
    if (!projectAiSourceCandidates.length) {
      setMessages((previous) => [
        ...previous,
        {
          role: "ai",
          text: "확인 가능한 Project Knowledge 또는 접근 가능한 회의기록 출처가 없어 답변할 수 없습니다.",
          tags: ["unsupported", "근거 없음"]
        }
      ]);
      setLoading(false);
      return;
    ...
    try {
      const response = await fetch(`${AI_API_BASE_URL}/api/meeting-ai/ask`, {
        ...
        body: JSON.stringify({
          question: trimmed,
          transcript: legacyProjectAiPayload.transcript,
          decisions: legacyProjectAiPayload.decisions,
          actions: legacyProjectAiPayload.actions
        })
      });
      ...
      setMessages((previous) => [
        ...previous,
        {
          ...
          text: result.answer,
          tags: [
            projectKnowledgeSources.length ? `Project Knowledge ${projectKnowledgeSources.length}` : "Project Knowledge 없음",
            accessibleMeetingSources.length ? `Meeting record ${accessibleMeetingSources.length}` : "Meeting record 없음"
          ]
        }
      ]);
    } catch (fetchError) {
      ...
      setMessages((previous) => [
        ...
        {
          role: "ai",
          text: `${projectPromptFallback} 현재는 AI 서버 연결을 확인해야 합니다. 근거 없는 추정 답변은 표시하지 않습니다.`,
          tags: ["unsupported", "확인 불가"]
        }
      ]);
    } finally {
      setLoading(false);
  ...
  function handleCreateMeeting(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!newMeetingTitle.trim() || !newMeetingAt) {
      return;
    }
            ...
            <section className="project-list-section project-operations-section">
              ...
              <form className="project-meeting-create-form" onSubmit={handleCreateMeeting}>
                ...
                <label>
                  ...
                  >
                    {members.map((member) => (
                      <option key={`new-meeting-member-${member.email}`} value={member.email}>
                        {member.name} · {member.spaceRole}
                      </option>
                    ))}
                  </select>
                </label>
                <button disabled={!newMeetingTitle.trim() || !newMeetingAt} type="submit">회의 생성</button>
              </form>
              ...
              {selectedMeeting ? (
                <>
                  ...
                  <div className="project-acl-note">
                    default-deny 기준 화면입니다. 명시 참여자만 회의 접근 대상으로 보이고, OWNER/ADMIN은 ACL 없이 override 접근으로 표시합니다.
                    ...
                    {visibleParticipants.map((participant) => (
                      <div key={participant.id} className="project-acl-row">
                        ...
                        <select
                          ...
                          disabled={isLastActiveHost(participant)}
                          onChange={(event) =>
            ...
            <section className="project-side-block ask">
              ...
              <div className="project-ai-source-panel">
                <div>
                  ...
                  <span>{projectAiPayload.projectKnowledge.length}개 공식 지식</span>
                </div>
                <div>
                  <strong>Meeting record</strong>
                  <span>{projectAiPayload.meetings.length}개 접근 가능 회의</span>
                </div>
                {!projectAiPayload.meetings.length ? (
                  <p>접근 가능한 회의기록 source가 없어 회의 기반 답변은 확인 불가로 표시됩니다.</p>
                ) : null}
              </div>
